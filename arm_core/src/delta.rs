...
pub struct DeltaWitness {
    /// The signing key.
    /// Corresponds to random commitment value of a witness resource.
...
pub struct DeltaInstance {
    /// The verifying key.
    /// Corresponds to the delta point sum.
...
impl DeltaProof {
    ...
    pub fn prove(message: &[u8], witness: &DeltaWitness) -> Result<DeltaProof, DeltaError> {
        ...
        if recid.to_byte() > 1 {
            return Err(DeltaError::InvalidDeltaProof);
        }
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<DeltaProof, DeltaError> {
        Ok(DeltaProof {
            signature: Signature::from_bytes((&bytes[0..64]).into())
                .map_err(|_| DeltaError::InvalidSignature)?,
            recid: RecoveryId::from_byte(bytes[64] - 27).ok_or(DeltaError::InvalidSignature)?,
        })
    }
}
...
impl DeltaWitness {
    ...
    pub fn from_bytes_vec(keys: &[Vec<u8>]) -> Result<DeltaWitness, DeltaError> {
        let witnesses: Result<Vec<DeltaWitness>, DeltaError> = keys
            .iter()
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<DeltaWitness, DeltaError> {
        Ok(DeltaWitness {
            signing_key: SigningKey::from_bytes(bytes.into())
                .map_err(|_| DeltaError::InvalidSigningKey)?,
        })
    }
...
impl DeltaInstance {
    ...
    pub fn from_deltas(deltas: &[ProjectivePoint]) -> Result<DeltaInstance, DeltaError> {
        let sum = deltas
            .iter()
            .fold(ProjectivePoint::IDENTITY, |acc, x| acc + x);
        let pk =
            PublicKey::from_affine(sum.to_affine()).map_err(|_| DeltaError::InvalidPublicKey)?;
